...
The final gradient boosting model gives an error rate of 0.143. The final extreme gradient boosting model also gives an error rate of 0.143.
...
## Model 5: Ensembled Model

Taking the majority vote of the result of previously built models gives us the fifth model. 
The miclassification error is about 0.154

```{r, include = F }
a <- apply(cbind(fit.rf.final.pred, as.character(xgb.pred), as.character(pred.gbm),logit.pred ), 1, table)
ensembled_error<-mean(sapply(a, function(x) names(x)[which(x>=2)[1]])!=data.test$AD)
```
...
testing_error = data.frame(
  Model = c("Logistic Model", "Random Rorest",  "Gradient Boosting Machine", "XG Boost", "Ensembled Model"),
  TestError = c(logit.error, fit.rf.test.err,gbm.error ,xgb.error,ensembled_error)
  )
testing_error
```
