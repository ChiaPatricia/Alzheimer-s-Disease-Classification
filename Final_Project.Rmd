...
```{r echo = F}
data %>% 
  ggplot(aes(x=MMSE,y=Age, color = AD, size = Visit)) +
...
In the end, we find that `mtry=4` gives the minimum error rate.
...
```{r, echo = F}
fit.rf.final <- randomForest(AD~., data.train, mtry = 4, 
...
Using 1/2 as threshold to determine the class of the instances in the test dataset, the misclassification rate is 0.145.
and AUC is 0.91. The confusion matrix is showing below and we can see that the model's ability to predict positive class or negative class is balanced.
