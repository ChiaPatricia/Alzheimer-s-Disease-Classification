...
g <- data %>% 
  ...
  labs(title = "Mini Mental State Examination Score, Age, eTIV and AD diagnosis",
...
data %>%
  ...
  stat_summary(fun.y=mean, geom="point", shape = 15, size=8) +
  geom_jitter(alpha = 0.4, color = "gray", size = 2) +
  theme_minimal() +
...
data %>%
  ...
  geom_boxplot(size = 1.5) +
  stat_summary(fun.y=mean, geom="point", shape = 15, size=8) +
  geom_jitter(alpha = 0.3, color = "gray", size = 2) +
  theme_minimal() +
...
In addition, age, nWBV (normalized whole brain volume) and eTIV (estimated total intracranial volume) also show significance. Subjects with higher age, smaller normalized whole brain volume and smaller estimated total intracranial volume have a higher chance of being diagnosed with dementia.
